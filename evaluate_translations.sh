...
do
  ...
  for set in thirdeye_phrase_against_none_weights_${w1}_${w2}_${w3};
  do
    # Build translations
    python3 $TOOLDIR/rejoin_oovs.py mudeval.align mud.oovlist.trans_$set /tmp/_tok_$set
    ...
    # Calculate Score
    printf "%-60s " $set
    java -jar $TOOLDIR/meteor-1.5/meteor-1.5.jar /tmp/_detok_$set $ref -l en -norm 2>/dev/null | grep '^Final score:' | sed 's/^Final score: *//' > results/score_${w1}_${w2}_${w3}
    # Cleanup
